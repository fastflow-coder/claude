...
def _normalize_category(cat: Optional[str]) -> str:
    if cat and cat.lower() in VALID_STOCK_CATEGORIES:
        return cat.lower()
    return "other"


def save_receipt(parsed: dict, image_path: Optional[str], telegram_msg_id: Optional[int]) -> Receipt:
    """Persist a parsed receipt (and auto-create linked expense row)."""
    import dateutil.parser as dp  # lazy import — only needed here
    ...
    with get_db() as db:
        ...
        for item_data in parsed.get("items") or []:
            item = ReceiptItem(
                ...
                stock_category=_normalize_category(item_data.get("stock_category")),
            )
            db.add(item)
...
def get_receipts(limit: int = 50, offset: int = 0, category: Optional[str] = None, search: Optional[str] = None) -> list[Receipt]:
    with get_db() as db:
...
def add_income_entry(date: datetime.date, amount: float, source: str = "daily_sales", note: Optional[str] = None) -> IncomeEntry:
    with get_db() as db:
...
def get_income_entries(limit: int = 50, offset: int = 0) -> list[IncomeEntry]:
    with get_db() as db:
        return db.query(IncomeEntry).order_by(IncomeEntry.date.desc()).offset(offset).limit(limit).all()
...
def get_weekly_summary() -> dict:
    ...
    return {
        ...
        "receipts_this_week": receipts_count,
...
def update_stock_quantity(item_name: str, quantity_delta: float, unit: Optional[str] = None) -> Optional[Stock]:
